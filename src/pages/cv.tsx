...
const JobSection: React.FC<IJobsProps> = (props) => {
  ...
  const firstElem = props.index === 0;
  return (
    <Row className={className}>
      <Main>
        {firstElem && (
          <div>
            <SubTitle> {props.name} </SubTitle> <p>{props.desc}</p> <hr />
          </div>
        )}
        {!firstElem && (
          <div>
            <SubTitle> Past Activity </SubTitle>{" "}
          </div>
        )}
        <RoleTitle>
          {job.current ? "Current Working " : ""}
          <a href={job.companyLink}>@{job.companyName}</a>
        </RoleTitle>
        <p>
          <img src={job.logoUrl} width="90px" />
        </p>
        <p>
...
const Cv: React.FC<InferGetStaticPropsType<typeof getStaticProps>> = ({
  cvData,
}) => {
  if (!cvData) {
    return (
