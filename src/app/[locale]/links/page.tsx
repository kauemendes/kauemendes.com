  ...
  return (
    <div className="min-h-screen bg-gradient-brand">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-16 pt-16">
          ...
          <div className="mb-8">
            <div className="relative inline-block">
              <div className="absolute -inset-4 bg-gradient-accent rounded-full opacity-20 blur-lg"></div>
              <Image
                src="/images/avatars/avatar.png"
                alt="Kaue Mendes"
                width={150}
                height={150}
                className="relative rounded-full mx-auto border-4 border-accent/30 shadow-2xl"
              />
            </div>
          </div>

          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold font-poppins text-ink mb-4">
            <span className="bg-gradient-accent bg-clip-text text-transparent">Kaue</span> Mendes
          </h1>

          <p className="text-ink-muted text-lg mb-4 max-w-2xl mx-auto leading-relaxed">
            {t('role')}
          </p>
            ...
            {socialLinks.map((link, index) => (
              <div key={link.name} className="animate-fadeIn" style={{ animationDelay: `${index * 0.1}s` }}>
                <Link
                  href={link.url}
                  target="_blank"
                  className="group block w-full p-6 bg-surface-raised/80 backdrop-blur rounded-xl border border-edge hover:border-accent/30 transition-all duration-300 shadow-lg hover:shadow-2xl hover:-translate-y-1"
                  onClick={() => handleLinkClick(link.name)}
                >
                  <div className="flex items-center space-x-6">
                    <div className="relative">
                      <div className="absolute -inset-2 bg-accent/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                      <Image 
                        src={link.icon} 
                        alt={link.name}
                        width={40} 
                    ...
                    <div className="flex-1">
                      <h3 className="font-bold text-ink text-lg group-hover:text-accent transition-colors duration-300">
                        {link.name}
                      </h3>
        ...
        {/* Call to Action */}
        <div className="relative bg-linear-to-r from-accent/15 to-accent-strong/15 rounded-2xl p-8 md:p-12 text-center border border-accent/30 overflow-hidden mb-16">
